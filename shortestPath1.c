...
void Dijkstra(graph G, int n, int s); // Dijkstra 알고리즘
// 힙
...
void Dijkstra(graph G, int n, int s) {
	...
	for (int i = 1; i <= n; i++) {
		...
		heap[i].distance = 30000; // MAX
		distance[i] = 30000; // 무한대
	}
	distance[s] = 0;
	...
	while (i > 0) {
		...
		while (e != NULL) {
			...
			if (includeVertex(heap, z, i) && // 정점 z가 heap에 존재, 즉 아직 방문 안함
				G.edges[e->edgeIdx].weight + distance[u] < distance[z]) { // 배낭에서 정점을 방문하는 데 더 짧은 길을 발견
				distance[z] = G.edges[e->edgeIdx].weight + distance[u]; // 거리 갱신
	...
	// 정점 번호의 오름차순으로 출력. 방문하지 않은 곳은 출력 X
	for (int j = 0; j < n; j++) {
		if (distance[j + 1] != 0 && distance[j + 1] != 30000)
			printf("%d %d\n", j + 1, distance[j + 1]);
	}
	// free
	free(heap);
